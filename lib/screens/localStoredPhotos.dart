...
class GetPhotosFromLocalStorage extends ConsumerStatefulWidget {
  //final List<Map<String, dynamic>> storedPhotos;
  const GetPhotosFromLocalStorage({super.key});

  @override
  ConsumerState<GetPhotosFromLocalStorage> createState() =>
      _GetPhotosFromLocalStorageState();
}

class _GetPhotosFromLocalStorageState
    extends ConsumerState<GetPhotosFromLocalStorage> {
  ...
  Future<List<Map<String, dynamic>>> get3yearStoredPhotos() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final encodedPhotos = prefs.getStringList('sstored_threeYearsPhoto') ?? [];
    final decodedPhotos =
        encodedPhotos.map((encodedPhoto) => jsonDecode(encodedPhoto)).toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        body: ref.read(photosNoInternetProvider).isEmpty
            ? Center(
                child: Text(
                  'No photos available.',
                      ...
                      SizedBox(
                        ...
                        child: car.CarouselSlider.builder(
                          ...
                          itemBuilder: (context, index, realIndex) {
                            final photoFrame = index <
                                    ref.read(photosNoInternetProvider).length
                                ? ref.read(photosNoInternetProvider)[index]
                                : '';
                            //  (selectedOption == 'Last In')
